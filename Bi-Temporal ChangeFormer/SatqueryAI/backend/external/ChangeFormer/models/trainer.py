...
class CDTrainer():
    ...
    def _load_checkpoint(self, ckpt_name='last_ckpt.pt'):
        ...
        if os.path.exists(ckpt_path):
            ...
            try:
                pretrain_dict = torch.load(self.args.pretrain, map_location=self.device, weights_only=False)
            except TypeError:
                pretrain_dict = torch.load(self.args.pretrain, map_location=self.device)
            self.net_G.load_state_dict(pretrain_dict, strict=False)
            self.net_G.to(self.device)
            self.net_G.eval()
        else:
